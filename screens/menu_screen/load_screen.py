...
class LoadScreen(MenuScreen):
    ...
    def handle_event(self, event: pygame.event.Event):
        if self.onceki_button.clicked(event, ScreenType.LOAD):
            self.change_page(-1)

        if self.sonraki_button.clicked(event, ScreenType.LOAD):
            self.change_page(1)

        if self.return_button.clicked(event, ScreenType.LOAD):
            self.game.screen_manager.set_screen(self.return_button.plus_data)
            return

        for slot in self.visible_slots:
            if slot.display.clicked(event, ScreenType.LOAD):
                slot: Slot
                ...
                if self.game.GAME_VERSION != version:
                    self.warning = True

                    self.game.screen_manager.set_screen(ScreenType.WARN, slot_id=slot.id)
                    return
